...
public class Mapping {
    ...
    public void mapping() {
        ...
        List<Employee> mappedEmployees = new MapHelper<>(employees)
                ...
                .map(e -> e.withJobHistory(incrementOneYear(e.getJobHistory()))) // Добавить всем сотрудникам 1 год опыта .map(e -> e.withJobHistory(incrementOneYear(e.getJobHistory())))
                .map(e -> e.withJobHistory(qaToQA(e.getJobHistory()))) // Заменить все qa на QA
    ...
    private List<JobHistoryEntry> qaToQA(List<JobHistoryEntry> jh) {
        return new MapHelper<>(jh).map(e -> e.getPosition().equals("qa") ? e.withPosition("QA") : e).getList();
    }

    private List<JobHistoryEntry> incrementOneYear(List<JobHistoryEntry> jh) {
    ...
    public void lazyMapping() {
        ...
        List<Employee> mappedEmployees = LazyMapHelper.from(employees)
                ...
                .map(e -> e.withJobHistory(incrementOneYear(e.getJobHistory()))) // Добавить всем сотрудникам 1 год опыта
                .map(e -> e.withJobHistory(qaToQA(e.getJobHistory()))) // Заменить все qu на QA
                .force();
